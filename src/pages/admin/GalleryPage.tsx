...
import { useGalleryItems, useCreateGalleryItem, useUpdateGalleryItem, useDeleteGalleryItem, GalleryItem } from "@/api/hooks/cms";
...
export default function GalleryPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const payload = { ...editing, image } as any;
    try {
      if (editing?.id) {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
      <Card className="xl:col-span-2">
        <CardContent className="p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Button onClick={() => setEditing({ title: "", category: "Farming & Agriculture", is_active: true, sort: 0 })}>
              + New
            </Button>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  {!items.length && <tr><td colSpan={7} className="py-6 text-center text-muted-foreground">No items.</td></tr>}
                </tbody>
              </table>
            </div>
          )}
      ...
      <Card>
        <CardContent className="p-6">
          <h3 className="text-lg font-semibold mb-4">{editing?.id ? "Edit Gallery Item" : "Create Gallery Item"}</h3>
          {!editing ? (
            ...
            <form className="space-y-4" onSubmit={onSubmit}>
              <div>
                ...
                <select className="w-full border rounded-md p-2 text-sm"
                  ...
                  onChange={(e)=>setEditing((p)=>({ ...p!, category: e.target.value as GalleryItem["category"] }))}>
                  {categories.map((c)=> <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <div>
              ...
              <div>
                ...
                <Input value={editing.date_label || ""} onChange={(e)=>setEditing((p)=>({ ...p!, date_label: e.target.value }))} />
              </div>
              <div>
              ...
              <div>
                ...
                <Input value={editing.attendees || ""} onChange={(e)=>setEditing((p)=>({ ...p!, attendees: e.target.value }))} />
              </div>
              <div>
                <Label>Image</Label>
