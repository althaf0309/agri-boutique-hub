...
export default function ProductsPage() {
  ...
  const params: Record<string, any> = {
    ...
    ordering,
    include_images: true, // request images if backend supports
  };
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  items.map((p: any) => {
                    ...
                    const image = getPrimaryProductImage(p);

                    const qty = p.quantity ?? (p.in_stock ? 1 : 0);
                    const inStock = (p.in_stock ?? qty > 0) as boolean;
                      ...
                      <TableRow key={p.id}>
                        ...
                        <TableCell className="hidden md:table-cell">
                          <div className="flex items-center gap-2">
                            <span className={`font-medium text-sm ${inStock ? 'text-green-600' : 'text-red-600'}`}>
                              {qty}
                            </span>
