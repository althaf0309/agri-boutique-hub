...
import { Plus, X, Package, Zap } from "lucide-react";
...
export function ProductOptions({ options, onOptionsChange, onGenerateVariants }: ProductOptionsProps) {
  ...
  const addOption = () => {
    ...
    const exists = options.some((o) => o.name.toLowerCase() === label.toLowerCase());
    if (exists) {
      toast({
    ...
    toast({ title: "Option added", description: `${newOption.name} added.` });
  };

  const removeOption = (index: number) => {
    const updated = options.filter((_, i) => i !== index);
    onOptionsChange(updated);
    toast({ title: "Option removed", description: "Option deleted." });
  };
  ...
  const addValue = (optionIndex: number) => {
    ...
    const exists = updated[optionIndex].values.some(
      (v) => v.toLowerCase() === valueToAdd.toLowerCase()
    );
    if (!exists) {
      updated[optionIndex].values.push(valueToAdd);
      onOptionsChange(updated);
  ...
  const addQuickOption = (optionName: string, suggestions: string[]) => {
    // avoid duplicate quick add
    const exists = options.some((o) => o.name.toLowerCase() === optionName.toLowerCase());
    if (exists) {
      toast({ title: "Already added", description: `${optionName} exists.` });
      return;
    }
    const newOption: Option = { id: Date.now(), name: optionName, values: suggestions };
    onOptionsChange([...options, newOption]);
    toast({
  ...
  const addSuggestionValue = (optionIndex: number, suggestion: string) => {
    const updated = [...options];
    if (!updated[optionIndex].values.includes(suggestion)) {
      updated[optionIndex].values.push(suggestion);
      onOptionsChange(updated);
    }
  };

  const totalCombos = options.reduce(
    (total, option) => total * Math.max(option.values.length, 1),
    1
  );
  const canGenerateVariants = options.some((o) => o.values.length > 0);
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 mt-2">
            {quickSuggestions.map((q) => (
              <Button
                ...
                variant="outline"
                size="sm"
                onClick={() => addQuickOption(q.name, q.suggestions)}
        ...
        {options.length > 0 && (
          <div className="space-y-4">
            {options.map((option, optionIndex) => (
              <Card key={option.id ?? `${option.name}-${optionIndex}`} className="p-4">
                <div className="space-y-3">
                  ...
                  {getCategorySuggestions(option.name).length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {getCategorySuggestions(option.name).map((s) => (
                          <Button
                            key={s}
                            variant="ghost"
                            size="sm"
                            className="h-6 px-2 text-xs"
                            onClick={() => addSuggestionValue(optionIndex, s)}
                            disabled={option.values.includes(s)}
                          >
                            {s}
                          </Button>
                        ))}
                  ...
                  <div className="flex flex-wrap gap-2">
                    {option.values.map((value, valueIndex) => (
                      <Badge key={`${value}-${valueIndex}`} variant="secondary" className="gap-1">
                        {value}
                        <Button
                          variant="ghost"
                          size="sm"
                  ...
                  <div className="flex gap-2">
                    <Input
                      ...
                      onKeyDown={(e) => e.key === "Enter" && addValue(optionIndex)}
                      className="h-8"
                    />
                    <Button
                      size="sm"
          ...
          <>
            ...
            <div className="space-y-4">
              ...
              <div className="p-3 bg-muted rounded-lg">
                ...
                <div className="mt-1 text-sm">{totalCombos} possible combinations</div>
                <div className="mt-2 flex flex-wrap gap-1">
                  {options.slice(0, 2).flatMap((option) =>
